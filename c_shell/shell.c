...
int main(int argc, char *argv[])
{
    ...
    int pidHistoryArray[MAX_NUM_PID];
    int historyCount = 0;
    ...
    //allocate memory for pid history array
    for (int i = 0; i< MAX_NUM_PID; i++)
    {
        //pidHistoryArray[i] = malloc (15 * sizeof(int)); //arbitrary amount of memory

        //setting each element to null because when you use the showpids command
        //it will only print the non-NULL elements
        pidHistoryArray[i] = 0; 
    }
    ...
    while( 1 )
    {
        ...
        //resets PID history so it only tracks most recent commands
        ...
        {
            //if "exit" or "quit" entered end program
            ...
            else
            {
                ...
                else if (strchr(token[0], '!'))
                {
                    ...
                    {
                        if (strlen(historyArray[k]) > 0)
                        {
                            printf("%d: %s", k, historyArray[k]);
                        }
                    }
                }
                //history command
                ...
                else if (strcmp(token[0], "showpids") == 0)
                {
                    ...
                    {
                        if (pidHistoryArray[k] > 0) //unused elements set to 0
                        {
                            printf("%d: %d\n", k, pidHistoryArray[k]);
                        }
                    }
